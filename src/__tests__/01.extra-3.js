...
import App from '../exercise/01.extra-3'
...
test('clicking the button increments the count with useReducer', async () => {
    ...
    const incrementObj = screen.getByTestId('incrementObj');
    await userEvent.click(incrementObj)
    expect(label).toHaveTextContent('1')

    const incrementFunc = screen.getByTestId('incrementFunc');
    await userEvent.click(incrementFunc)
